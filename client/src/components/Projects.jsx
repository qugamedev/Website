...
import axios from 'axios'; // For the database connection
    ...
    <div className="bg-[#0f0f10] min-h-screen text-white font-sans pt-24">
      ...
      <div className="pb-6">
        <div className="grid md:grid-cols-3 grid-cols-1 justify-center items-center">
          {/* Project Example*/}
          {/* use col-start to position the game in the grid*/}
          <div role="button" onClick={handleClick} className="md:col-start-2 relative px-5 pb-5 mx-auto duration-300 ease-out hover:scale-110">
            {/*Project image */}
            <img className="relative inset-0 z-30 bg-gray-800 md:size-64 size-40 rounded-sm object-cover object-left"
              src="src\assets\images\tempArrowDynamix.jpg"></img>
            {/* Display from database, use this code instead 
            src={data.gameImg} */}
            ...
            {/* Project Title */}
            <div className="py-4 text-center md:text-3xl text-2xl font-bold text-white opacity-70">
              Arrow Dynamix</div>
            {/* Display from database, use this code instead
            {data.gameTitle} */}
          </div>
      ...
      {/*Game project description pop-up */}
      {
        ...
        <div
          ...
          className="sticky bottom-0 pb-4 z-40">
          <div className="sticky relative flex flex-col mx-auto w-5/6">
            ...
            <div className="relative z-30 h-100 rounded-xl bg-gray-800 border-8 border-purple-600">
              {/*text title/desc game*/}
              <div className="text-purple-200 p-5 ">
                <span className="underline text-cyan-200 hover:text-cyan-500 ease-out duration-300 underline-offset-4 font-bold md:text-xl text-2xl"><a href="
                https://chocokari.itch.io/arrow-dynamix  
                " target="_blank">
                  {/* Display from database, use this code instead
            {data.gameUrl} */}
                  Arrow Dynamix
                  {/* Display from database, use this code instead
            {data.gameTitle} */}</a></span>
                <div className="pt-1 italic text-[12pt]">
                  Kari, Froggy, Colon, and Sungpoon
                  {/* Display from database, use this code instead
            {data.gameDevs} */}</div>
                <p className="text-[12pt]">
                  -------</p>
                <div className="overflow-y-scroll h-32 text-[12pt]">
                  Use your bow and arrow to solve unique platforming puzzles and escape the dungeon!
                  {/* Display from database, use this code instead
            {data.gameDesc} */}
